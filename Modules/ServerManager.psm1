...
function _GetProfileListeningPorts {
    param([hashtable]$Profile)
    ...
    function _AddPort {
        ...
        $port = 0
        if ([int]::TryParse($text, [ref]$port) -and $port -gt 0 -and $port -le 65535 -and $seen.Add($port)) {
            $ports.Add($port) | Out-Null
        }
    }

    foreach ($name in @('ServerPort','GamePort','Port','QueryPort','SatisfactoryApiPort')) {
        try {
            if ($Profile.ContainsKey($name)) { _AddPort $Profile[$name] }
        } catch { }
    }

    $rconEnabled = $false
    try {
        $rconPassword = _CoalesceStr $Profile.RconPassword ''
        $rconEnabled = (-not [string]::IsNullOrWhiteSpace($rconPassword))
    } catch { $rconEnabled = $false }
    if ($rconEnabled) {
        try {
            if ($Profile.ContainsKey('RconPort')) { _AddPort $Profile.RconPort }
        } catch { }
    }

    $telnetEnabled = $false
    try {
        $telnetHost = _CoalesceStr $Profile.TelnetHost ''
        $telnetPassword = _CoalesceStr $Profile.TelnetPassword ''
        $telnetEnabled = ((-not [string]::IsNullOrWhiteSpace($telnetHost)) -or (-not [string]::IsNullOrWhiteSpace($telnetPassword)))
    } catch { $telnetEnabled = $false }
    if ($telnetEnabled) {
        try {
            if ($Profile.ContainsKey('TelnetPort')) { _AddPort $Profile.TelnetPort }
        } catch { }
    }

    $restEnabled = $false
    try { $restEnabled = [bool]$Profile.RestEnabled } catch { $restEnabled = $false }
    if ($restEnabled) {
        try {
            if ($Profile.ContainsKey('RestPort')) { _AddPort $Profile.RestPort }
        } catch { }
    }

    if (_TestProfileGame -Profile $Profile -KnownGame 'Minecraft') {
        $folderPath = ''
        ...
        if (-not [string]::IsNullOrWhiteSpace($folderPath)) {
            $serverProps = Join-Path $folderPath 'server.properties'
            if (Test-Path -LiteralPath $serverProps) {
                try {
                    foreach ($line in @(Get-Content -LiteralPath $serverProps -ErrorAction Stop)) {
                        if ($line -match '^\s*(server-port|query.port|rcon.port)\s*=\s*(\d+)\s*$') {
                            _AddPort $Matches[2]
                        }
                    }
                } catch { }
...
function _FindListeningProcessForProfile {
    param([hashtable]$Profile)

    $ports = @(_GetProfileListeningPorts -Profile $Profile)
    if (-not $ports -or $ports.Count -eq 0) { return $null }
    ...
    foreach ($port in $ports) {
        ...
        try {
            ...
            if ($conn -and $conn.OwningProcess) { $ownerPid = [int]$conn.OwningProcess }
        } catch { }

        if (-not $ownerPid) {
            try {
                $lines = @(netstat -ano -p tcp 2>$null | Select-String -Pattern "LISTENING\s+(\d+)\s*$")
                foreach ($line in $lines) {
                    $text = "$($line.Line)"
                    if ($text -match "[:\.]$port\s+\S+\s+LISTENING\s+(\d+)\s*$") {
...
function _FindLiveProcessForProfile {
    param([hashtable]$Profile)
    ...
    $primaryProcessName = _CoalesceStr $Profile.ProcessName ''
    if (-not [string]::IsNullOrWhiteSpace($primaryProcessName) -and $seen.Add($primaryProcessName)) {
        $candidateNames.Add($primaryProcessName) | Out-Null
    }

    foreach ($hint in @(_GetProfileExeHints -Profile $Profile)) {
        if ([string]::IsNullOrWhiteSpace($hint)) { continue }
        $baseName = ''
        try { $baseName = [System.IO.Path]::GetFileNameWithoutExtension([string]$hint) } catch { $baseName = [string]$hint }
        if (-not [string]::IsNullOrWhiteSpace($baseName) -and $seen.Add($baseName)) {
            $candidateNames.Add($baseName) | Out-Null
        }
    }
    ...
    foreach ($name in @($candidateNames)) {
        try {
            foreach ($proc in @(Get-Process -Name $name -ErrorAction SilentlyContinue)) {
                ...
                $nameLower = ''
                try { $nameLower = $proc.ProcessName.ToLowerInvariant() } catch { $nameLower = '' }
                $folderMatched = $false
                if ($needleFolder) {
                    if ($procPath -and $procPath.StartsWith($needleFolder, [System.StringComparison]::OrdinalIgnoreCase)) {
                        $folderMatched = $true
                    } elseif ($cmdLine -and $cmdLine.IndexOf($needleFolder, [System.StringComparison]::OrdinalIgnoreCase) -ge 0) {
                        $folderMatched = $true
                    }
                }

                # Never claim generic host processes unless they also point back to this profile.
                if (($genericNames -contains $nameLower) -and -not $folderMatched) { continue }
    ...
    $portProc = _FindListeningProcessForProfile -Profile $Profile
    if ($portProc -and -not $portProc.HasExited) {
        $portProcName = ''
        $portProcPath = ''
        $portProcCmdLine = ''
        $portProcFolderMatched = $false
        try { $portProcName = $portProc.ProcessName.ToLowerInvariant() } catch { $portProcName = '' }
        try { $portProcPath = "$($portProc.Path)" } catch { $portProcPath = '' }
        try {
            $cim = Get-CimInstance Win32_Process -Filter "ProcessId = $($portProc.Id)" -ErrorAction Stop
            if ($cim) {
                if (-not $portProcPath) { $portProcPath = "$($cim.ExecutablePath)" }
                $portProcCmdLine = "$($cim.CommandLine)"
            }
        } catch { }

        if ($needleFolder) {
            if ($portProcPath -and $portProcPath.StartsWith($needleFolder, [System.StringComparison]::OrdinalIgnoreCase)) {
                $portProcFolderMatched = $true
            } elseif ($portProcCmdLine -and $portProcCmdLine.IndexOf($needleFolder, [System.StringComparison]::OrdinalIgnoreCase) -ge 0) {
                $portProcFolderMatched = $true
            }
        }

        if (($genericNames -contains $portProcName) -and -not $portProcFolderMatched) {
            _LogThrottled -Key "FindLiveProcessPortFallback::$($Profile.GameName)::PID$($portProc.Id)" -Msg "[$($Profile.GameName)] Ignored generic process '$($portProc.ProcessName)' (PID $($portProc.Id)) from port fallback because it does not match the profile folder." -Level WARN -WindowSeconds 30
            return $null
        }
...
function Sync-RunningServersFromProcesses {
    ...
    function _GetProcessMatchScore {
        ...
        $genericNames = @('java','javaw','powershell','pwsh','cmd','conhost')
        $processNameLower = ''
        try { $processNameLower = $Process.ProcessName.ToLowerInvariant() } catch { $processNameLower = '' }
        $isGenericHost = ($genericNames -contains $processNameLower)

        if ($exeName -and -not $isGenericHost) {
        ...
        if ($exeHints.Count -gt 0) {
            ...
            foreach ($exeHint in @($exeHints)) {
                $hintValue = ''
                try { $hintValue = [string]$exeHint } catch { $hintValue = '' }
                $hintIsGeneric = ($genericNames -contains $hintValue.ToLowerInvariant())
                if ($hintIsGeneric) { continue }
    ...
    foreach ($pfx in @($SharedState.Profiles.Keys)) {
        ...
        if ($SharedState.RunningServers.ContainsKey($pfx)) {
            ...
            $live = $null
            try {
                $live = Get-Process -Id $entry.Pid -ErrorAction Stop
            } catch {
                $live = $null
            }
            ...
            $tracePid = $null
            $traceStart = $null
            try { $tracePid = $entry.Pid } catch { $tracePid = $null }
            try { $traceStart = $entry.StartTime } catch { $traceStart = $null }
        ...
        if (-not $procName) {
            $portProc = _FindListeningProcessForProfile -Profile $profile
            if ($portProc -and -not $portProc.HasExited -and -not $claimedPids.Contains([int]$portProc.Id)) {
                $procs = @($portProc)
            } else {
                continue
            }
        ...
        $portMatchedPid = $null
        try {
            $portProcForScore = _FindListeningProcessForProfile -Profile $profile
            if ($portProcForScore) { $portMatchedPid = [int]$portProcForScore.Id }
        } catch { $portMatchedPid = $null }
        ...
        $candidates = foreach ($candidate in $procs) {
            ...
            $candidateStart = Get-Date 0
            try { $candidateStart = $candidate.StartTime } catch { $candidateStart = Get-Date 0 }
            [pscustomobject]@{
                Process = $candidate
                Score   = $score
...
if ($null -eq $script:RuntimeTransitionStats) {
    $script:RuntimeTransitionStats = @{
        ChangedNone = 0
        ChangedDetailOnly = 0
        RealChange = 0
        LastLoggedAt = $null
...
function _TraceServerRuntimeStateTransition {
    ...
    try { $oldCodeRaw = $OldEntry.Code } catch { $oldCodeRaw = $null }
    try { $oldBadgeRaw = $OldEntry.Badge } catch { $oldBadgeRaw = $null }
    try { $oldLevelRaw = $OldEntry.Level } catch { $oldLevelRaw = $null }
    try { $oldDetail = $OldEntry.Detail } catch { $oldDetail = $null }

    try { $newCodeRaw = $NewEntry.Code } catch { $newCodeRaw = $null }
    try { $newBadgeRaw = $NewEntry.Badge } catch { $newBadgeRaw = $null }
    try { $newLevelRaw = $NewEntry.Level } catch { $newLevelRaw = $null }
    try { $newDetail = $NewEntry.Detail } catch { $newDetail = $null }

    $oldCode   = _NormalizeRuntimeTransitionToken $oldCodeRaw
    $oldBadge  = _NormalizeRuntimeTransitionToken $oldBadgeRaw
    $oldLevel  = _NormalizeRuntimeTransitionToken $oldLevelRaw

    $newCode   = _NormalizeRuntimeTransitionToken $newCodeRaw
    $newBadge  = _NormalizeRuntimeTransitionToken $newBadgeRaw
    $newLevel  = _NormalizeRuntimeTransitionToken $newLevelRaw

    if ($Action -eq 'set') {
        $oldSince = $null
        $newSince = $null
        try { $oldSince = [datetime]$OldEntry.Since } catch { $oldSince = $null }
        try { $newSince = [datetime]$NewEntry.Since } catch { $newSince = $null }
        $noStateChange =
    ...
    $transitionBucket = 'RealChange'
    if ($changeSummary -eq 'none') {
        $transitionBucket = 'ChangedNone'
    } elseif ($changed.Count -eq 1 -and $changed[0] -eq 'detail') {
        $transitionBucket = 'ChangedDetailOnly'
    }

    try {
        if (-not $script:RuntimeTransitionStats) {
            $script:RuntimeTransitionStats = @{
                ChangedNone = 0
                ChangedDetailOnly = 0
                RealChange = 0
        ...
        $script:RuntimeTransitionStats[$transitionBucket] = [int]$script:RuntimeTransitionStats[$transitionBucket] + 1
        $statsNow = Get-Date
        $lastLoggedAt = $null
        try { $lastLoggedAt = $script:RuntimeTransitionStats.LastLoggedAt } catch { $lastLoggedAt = $null }
        if (-not ($lastLoggedAt -is [datetime]) -or (($statsNow - $lastLoggedAt).TotalSeconds -ge 30)) {
            $script:RuntimeTransitionStats.LastLoggedAt = $statsNow
            _Log ('STATECOUNTS none={0} detailOnly={1} real={2}' -f `
                $script:RuntimeTransitionStats.ChangedNone, `
                $script:RuntimeTransitionStats.ChangedDetailOnly, `
                $script:RuntimeTransitionStats.RealChange) -Level DEBUG
        }
    } catch { }
...
function Set-JoinableServerRuntimeState {
    ...
    $runtimeCode = $runtimeCode.ToLowerInvariant()
    if (-not $Force -and $currentCount -le 0 -and $runtimeCode -in @('waiting_first_player','idle_wait','idle_shutdown','waiting_restart')) {
        return
    }
...
function _MaybePromoteJoinableFromCommandResponse {
    param(
        [string]$Prefix,
        [string]$Signal,
        [hashtable]$SharedState = $script:State
    ...
    if ([string]::IsNullOrWhiteSpace($Prefix) -or [string]::IsNullOrWhiteSpace($Signal) -or -not $SharedState) { return }
    Initialize-ServerManager -SharedState $SharedState

    $key = $Prefix.ToUpperInvariant()
    $profile = _GetProfile $key
    if (-not $profile -or -not (_ProfileUsesDeferredReadySignal -Profile $profile)) { return }
    if (-not $SharedState.RunningServers.ContainsKey($key)) { return }
    ...
    $runtimeCode = $runtimeCode.ToLowerInvariant()
    if ($runtimeCode -notin @('starting','waiting_first_player','idle_wait')) { return }

    $normalizedSignal = "$Signal".Trim().ToLowerInvariant()
    if ([string]::IsNullOrWhiteSpace($normalizedSignal)) { return }

    if (-not $SharedState.ContainsKey('CommandReadySignalState') -or -not $SharedState.CommandReadySignalState) {
        $SharedState['CommandReadySignalState'] = [hashtable]::Synchronized(@{})
    }

    $entry = $null
    if ($SharedState.CommandReadySignalState.ContainsKey($key)) {
        $entry = $SharedState.CommandReadySignalState[$key]
    }
    ...
    } elseif ($entry.Commands -isnot [System.Collections.Generic.HashSet[string]]) {
        $rehydratedCommands = New-Object 'System.Collections.Generic.HashSet[string]' ([System.StringComparer]::OrdinalIgnoreCase)
        foreach ($cmd in @($entry.Commands)) {
            if (-not [string]::IsNullOrWhiteSpace([string]$cmd)) { [void]$rehydratedCommands.Add([string]$cmd) }
        }
        $entry.Commands = $rehydratedCommands
    }

    [void]$entry.Commands.Add($normalizedSignal)
    $entry.Count = [int]$entry.Count + 1
    $entry.LastAt = Get-Date

    $promotionThreshold = 2
    if ($normalizedSignal -eq 'pz-players-header') {
        $promotionThreshold = 1
    }

    $shouldPromote = (-not [bool]$entry.Promoted) -and ([int]$entry.Count -ge $promotionThreshold)
    if ($shouldPromote) {
        $detail = "Server produced repeated player/query responses and is assumed joinable pending log confirmation."
        try {
            Set-JoinableServerRuntimeState -Prefix $key -Force -Detail $detail -SharedState $SharedState
            try {
                if ($SharedState.ContainsKey('ServerStartNotified') -and $SharedState.ServerStartNotified) {
                    $SharedState.ServerStartNotified[$key] = $true
                }
            } catch { }
            try {
                Send-DiscordGameEvent -Profile $profile -Prefix $key -Event 'joinable' -Tag 'JOINABLE' -SharedState $SharedState | Out-Null
            } catch {
                $suppressJoinable = $false
                try {
                    $suppressJoinable = [bool](_ShouldSuppressDiscordLifecycleWebhook -Prefix $key -Tag 'JOINABLE' -SharedState $SharedState)
                } catch { $suppressJoinable = $false }

                if (-not $suppressJoinable) {
                    try { _SendDiscordNotice (New-DiscordGameMessage -Profile $profile -Prefix $key -Event 'joinable') } catch { }
                }
            }
            $entry.Promoted = $true
            _Log "[$($profile.GameName)] Promoted to joinable after $($entry.Count) successful response signal(s): $(@($entry.Commands) -join ', ')." -Level INFO
        } catch { }
    }

    $SharedState.CommandReadySignalState[$key] = $entry
}
...
function _HandleStartupFailure {
    ...
    try {
        Invoke-SafeShutdown -Prefix $prefixKey -Quiet -SkipInGameAnnouncements | Out-Null
    } catch {
...
function _Trace7DaysToDieDebug {
    param(
        [string]$Prefix,
        [string]$Message
    )

    if ([string]::IsNullOrWhiteSpace($Prefix) -or [string]::IsNullOrWhiteSpace($Message)) { return }
    try {
        if ($Prefix.ToUpperInvariant() -ne 'DZ') { return }
    } catch { return }

    try { _Log "[7DTDDBG][$Prefix] $Message" -Level DEBUG } catch { }
}

function _SyncPlayerActivityFromSnapshot {
    param(
        [string]$Prefix,
        [hashtable]$Profile,
        [hashtable]$SharedState,
    ...
    try {
        ...
        $resolvedCount = [Math]::Max(0, [int]$Count)
        $activity.DetectionSupported = $true
        $activity.DetectionAvailable = $true
        $activity.CurrentCount = $resolvedCount
        $activity.LastCount = $resolvedCount
        $activity.LastObservedAt = Get-Date
        if ($resolvedCount -gt 0) {
            if ($null -eq $activity.FirstPlayerSeenAt) { $activity.FirstPlayerSeenAt = Get-Date }
            $activity.LastNonZeroAt = Get-Date
            $activity.LastBecameEmptyAt = $null
        } else {
            if ($null -ne $activity.FirstPlayerSeenAt -and $null -eq $activity.LastBecameEmptyAt) {
...
function _GetInGameAnnouncementTransport {
    param([hashtable]$Profile)

    if (-not $Profile) { return '' }

    if (_TestProfileGame -Profile $Profile -KnownGame 'Minecraft')      { return 'stdin' }
    if (_TestProfileGame -Profile $Profile -KnownGame '7DaysToDie')     { return 'telnet' }
    if (_TestProfileGame -Profile $Profile -KnownGame 'ProjectZomboid') { return 'stdin' }
    if (_TestProfileGame -Profile $Profile -KnownGame 'Palworld')       { return 'stdin' }

    return ''
}

function _GetInGameAnnouncementMessage {
    param(
        [hashtable]$Profile,
        [ValidateSet('save','shutdown','restart')]
        [string]$Kind,
        [int]$Seconds = 0
    )
    ...
    if (_TestProfileGame -Profile $Profile -KnownGame 'Minecraft') {
        switch ($Kind) {
            'save'     { return '[ECC NOTICE] Compliance requires a world save. Please continue mining in an orderly fashion.' }
            'shutdown' {
                if ($Seconds -eq 30) { return '[ECC NOTICE] Corporate has ordered this realm closed in 30 seconds. Please secure your belongings.' }
                return "[ECC NOTICE] Server shutdown in $Seconds second$(if ($Seconds -ne 1) { 's' } else { '' })."
            }
            'restart'  {
                if ($Seconds -eq 30) { return '[ECC NOTICE] Corporate maintenance begins in 30 seconds. Reality will resume shortly.' }
                return "[ECC NOTICE] Server restart in $Seconds second$(if ($Seconds -ne 1) { 's' } else { '' })."
            }
        }
    }
    ...
    if (_TestProfileGame -Profile $Profile -KnownGame '7DaysToDie') {
        switch ($Kind) {
            'save'     { return '[ECC NOTICE] Management has requested a survival record backup. Save starting now.' }
            'shutdown' {
                if ($Seconds -eq 30) { return '[ECC NOTICE] This sector will be closed in 30 seconds by order of management. Please conclude all screaming.' }
                return "[ECC NOTICE] Server shutdown in $Seconds second$(if ($Seconds -ne 1) { 's' } else { '' })."
            }
            'restart'  {
                if ($Seconds -eq 30) { return '[ECC NOTICE] Upper management has approved a wasteland reset in 30 seconds.' }
                return "[ECC NOTICE] Server restart in $Seconds second$(if ($Seconds -ne 1) { 's' } else { '' })."
            }
        }
    }
    ...
    if (_TestProfileGame -Profile $Profile -KnownGame 'Palworld') {
        switch ($Kind) {
            'save'     { return '[ECC NOTICE] Administrative preservation of company assets is now in progress.' }
            'shutdown' {
                if ($Seconds -eq 30) { return '[ECC NOTICE] This facility will close in 30 seconds by executive order.' }
                return "[ECC NOTICE] Server shutdown in $Seconds second$(if ($Seconds -ne 1) { 's' } else { '' })."
            }
            'restart'  {
                if ($Seconds -eq 30) { return '[ECC NOTICE] Mandatory facility restart in 30 seconds. Please remain employable.' }
                return "[ECC NOTICE] Server restart in $Seconds second$(if ($Seconds -ne 1) { 's' } else { '' })."
            }
        }
    }
    ...
    if (_TestProfileGame -Profile $Profile -KnownGame 'ProjectZomboid') {
        switch ($Kind) {
            'save'     { return '[ECC NOTICE] Records update in progress. Please try not to perish during archival operations.' }
            'shutdown' {
                if ($Seconds -eq 30) { return '[ECC NOTICE] This county will be closed in 30 seconds by corporate instruction.' }
                return "[ECC NOTICE] Server shutdown in $Seconds second$(if ($Seconds -ne 1) { 's' } else { '' })."
            }
            'restart'  {
                if ($Seconds -eq 30) { return '[ECC NOTICE] Mandatory server restart in 30 seconds. Further panic is not required.' }
                return "[ECC NOTICE] Server restart in $Seconds second$(if ($Seconds -ne 1) { 's' } else { '' })."
            }
        }
    }
...
function _ShouldSendInGameAnnouncement {
    param(
        [string]$Prefix,
        [hashtable]$Profile,
        [hashtable]$SharedState = $script:State
    ...
    if ([string]::IsNullOrWhiteSpace($Prefix) -or -not $Profile -or -not $SharedState) { return $false }
    if ([string]::IsNullOrWhiteSpace((_GetInGameAnnouncementTransport -Profile $Profile))) { return $false }

    $key = $Prefix.ToUpperInvariant()
    if (-not $SharedState.ContainsKey('RunningServers') -or -not $SharedState.RunningServers.ContainsKey($key)) { return $false }

    try {
        $activity = _UpdatePlayerActivityState -Prefix $key -Profile $Profile -SharedState $SharedState
        if ($activity -and $activity.DetectionAvailable) {
            return ([int]$activity.CurrentCount -gt 0)
        }
    } catch { }
...
function _SendInGameAnnouncement {
    param(
        [string]$Prefix,
        [hashtable]$Profile,
        [ValidateSet('save','shutdown','restart')]
    ...
    if ([string]::IsNullOrWhiteSpace($Prefix) -or -not $Profile) { return $false }

    $key = $Prefix.ToUpperInvariant()
    $message = _GetInGameAnnouncementMessage -Profile $Profile -Kind $Kind -Seconds $Seconds
    if ([string]::IsNullOrWhiteSpace($message)) { return $false }

    $transport = _GetInGameAnnouncementTransport -Profile $Profile
    ...
    if (_TestProfileGame -Profile $Profile -KnownGame 'Minecraft') {
        $command = "say $safeMessage"
    } elseif (_TestProfileGame -Profile $Profile -KnownGame '7DaysToDie') {
        $command = "say `"$safeMessage`""
    } elseif (_TestProfileGame -Profile $Profile -KnownGame 'Palworld') {
        $command = "broadcast $safeMessage"
    } elseif (_TestProfileGame -Profile $Profile -KnownGame 'ProjectZomboid') {
        $command = "servermsg `"$safeMessage`""
    }
    ...
    switch ($transport) {
        'stdin' {
            try { $ok = [bool](Send-ServerStdin -Prefix $key -Command $command) } catch { $ok = $false }
        }
        'telnet' {
            ...
            $telnetPass = if ($null -ne $Profile.TelnetPassword) { "$($Profile.TelnetPassword)".Trim() } else { '' }
            if ([string]::IsNullOrWhiteSpace($telnetPass)) {
                _Log "[$($Profile.GameName)] In-game announcement skipped because telnet is not configured." -Level WARN
                return $false
            }
            try {
                $telnetResult = Invoke-TelnetCommand -Host $telnetHost -Port $telnetPort -Password $telnetPass -Command $command
                $ok = [bool]($telnetResult -and $telnetResult.Success)
            } catch {
                $ok = $false
            }
        }
    ...
    if ($ok) {
        _Log "[$($Profile.GameName)] In-game $Kind announcement sent: $safeMessage"
    } else {
        _Log "[$($Profile.GameName)] In-game $Kind announcement failed: $safeMessage" -Level WARN
    }

    return $ok
}

function _InvokeInGameAnnouncementCountdown {
    param(
        [string]$Prefix,
        [hashtable]$Profile,
    ...
    if (-not (_ShouldSendInGameAnnouncement -Prefix $Prefix -Profile $Profile -SharedState $SharedState)) {
        return $false
    }

    $key = $Prefix.ToUpperInvariant()
    $thresholds = @(30, 20, 10, 5, 4, 3, 2, 1)

    for ($i = 0; $i -lt $thresholds.Count; $i++) {
        if (-not $SharedState -or -not $SharedState.ContainsKey('RunningServers') -or -not $SharedState.RunningServers.ContainsKey($key)) {
            return $false
        }

        $seconds = [int]$thresholds[$i]
        [void](_SendInGameAnnouncement -Prefix $key -Profile $Profile -Kind $Kind -Seconds $seconds)

        $sleepSeconds = if ($i -lt ($thresholds.Count - 1)) {
            [int]($thresholds[$i] - $thresholds[$i + 1])
        } else {
            1
        }
...
function _ParseMinecraftPlayersText {
    param([string]$Text)
    ...
    if ([string]::IsNullOrWhiteSpace($Text)) {
        $result.Note = 'No Minecraft player text was available.'
        return $result
    }
    ...
    for ($idx = $lines.Count - 1; $idx -ge 0; $idx--) {
        ...
        if ($line -match '(?i)\bthere are\s+(\d+)\s+of a max of\s+(\d+)\s+players?\s+online\s*:\s*(.*)$') {
            $count = 0
            try { $count = [int]$Matches[1] } catch { $count = 0 }
            $tail = ''
            try { $tail = [string]$Matches[3] } catch { $tail = '' }
            ...
            $result.Available = $true
            $result.Count = [Math]::Max($count, $names.Count)
            $result.Names = @($names)
            $result.Note = 'Player count observed from the Minecraft list response.'
            return $result
        }

        if ($line -match '(?i)\bno players\b|\bno one online\b') {
            $result.Available = $true
            $result.Count = 0
            $result.Names = @()
            $result.Note = 'Player count observed from the Minecraft list response.'
    ...
    $result.Note = 'Minecraft player text did not include a recognizable list response yet.'
    return $result
}
...
function _Refresh7DaysToDiePlayers {
    ...
    $telnetPort = if ($null -ne $Profile.TelnetPort) { [int]$Profile.TelnetPort } else { 8081 }
    $telnetHost = if ($null -ne $Profile.TelnetHost -and "$($Profile.TelnetHost)".Trim() -ne '') { "$($Profile.TelnetHost)".Trim() } else { '127.0.0.1' }
    if ([string]::IsNullOrWhiteSpace($telnetPass) -or $telnetPort -le 0) {
        _Trace7DaysToDieDebug -Prefix $key -Message ("poll skipped reason=missing-telnet-config host={0} port={1} hasPassword={2}" -f $telnetHost, $telnetPort, $(-not [string]::IsNullOrWhiteSpace($telnetPass)))
        return $false
    }
    ...
    if (-not $Force -and $pollState.LastAttemptAt) {
        try {
            if (($now - [datetime]$pollState.LastAttemptAt).TotalSeconds -lt $refreshIntervalSeconds) {
                _Trace7DaysToDieDebug -Prefix $key -Message ("poll skipped reason=throttled secondsSinceLastAttempt={0} refreshIntervalSeconds={1}" -f `
                    [Math]::Round(($now - [datetime]$pollState.LastAttemptAt).TotalSeconds, 1), $refreshIntervalSeconds)
                return $false
            }
        } catch { }
    }
    ...
    _Trace7DaysToDieDebug -Prefix $key -Message ("poll begin host={0} port={1} force={2}" -f $telnetHost, $telnetPort, $Force.IsPresent)
    ...
    if (-not $t.Success) {
        ...
        _Trace7DaysToDieDebug -Prefix $key -Message ("poll failed source=telnet error={0}" -f $pollState.Note)
        return $false
    }

    $responseLength = 0
    try {
        $responseLength = ([string]$t.Response).Length
    } catch { $responseLength = 0 }
    _Trace7DaysToDieDebug -Prefix $key -Message ("poll telnet-success responseLength={0}" -f $responseLength)
    ...
    _Trace7DaysToDieDebug -Prefix $key -Message ("poll parsed available={0} count={1} names={2} usedLogFallback={3} note={4}" -f `
        $parsed.Available, $parsed.Count, @($parsed.Names).Count, $usedLogFallback, $parsed.Note)
    ...
    _Trace7DaysToDieDebug -Prefix $key -Message ("poll cached count={0} names={1}" -f [Math]::Max(0, [int]$parsed.Count), @($parsed.Names).Count)
    try { _MaybePromoteJoinableFromCommandResponse -Prefix $key -Signal '7dtd-listplayers' -SharedState $SharedState } catch { }
...
function _RefreshHytalePlayers {
    ...
    _SetLatestPlayersSnapshot -Prefix $key -Names @($parsed.Names) -Count ([int]$parsed.Count) -SharedState $SharedState
    try { _MaybePromoteJoinableFromCommandResponse -Prefix $key -Signal 'hytale-who' -SharedState $SharedState } catch { }
    $pollState.LastSuccessAt = $now
    return $true
}

function _RefreshMinecraftPlayers {
    param(
        [string]$Prefix,
        [hashtable]$Profile,
        [hashtable]$SharedState,
    ...
    if (-not $Profile -or -not $SharedState) { return $false }
    if (-not (_TestProfileGame -Profile $Profile -KnownGame 'Minecraft')) { return $false }
    ...
    if (-not $Force) {
        if ($startupAt) {
            try {
                if (($now - $startupAt).TotalSeconds -lt $refreshIntervalSeconds) {
                    $pollState.Source = 'MinecraftHeartbeat'
                    $pollState.Note = ("Initial Minecraft player poll deferred until server uptime reaches {0} seconds." -f $refreshIntervalSeconds)
                    return $false
                }
            } catch { }
        }

        if ($pollState.LastAttemptAt) {
            try {
                if (($now - [datetime]$pollState.LastAttemptAt).TotalSeconds -lt $refreshIntervalSeconds) {
                    return $false
                }
    ...
    $parsed = $null
    $usedSource = ''
    $rconPass = if ($null -ne $Profile.RconPassword) { "$($Profile.RconPassword)".Trim() } else { '' }
    $rconPort = if ($null -ne $Profile.RconPort) { [int]$Profile.RconPort } else { 25575 }
    $rconHost = if ($null -ne $Profile.RconHost -and "$($Profile.RconHost)".Trim() -ne '') { "$($Profile.RconHost)".Trim() } else { '127.0.0.1' }

    if (-not [string]::IsNullOrWhiteSpace($rconPass) -and $rconPort -gt 0) {
        $r = Invoke-RconCommand -Host $rconHost -Port $rconPort -Password $rconPass -Command 'list'
        $usedSource = 'MinecraftRcon'
        if ($r.Success) {
            $parsed = _ParseMinecraftPlayersText -Text $r.Response
            if ($parsed.Available) {
                _SetLatestPlayersSnapshot -Prefix $key -Names @($parsed.Names) -Count ([int]$parsed.Count) -SharedState $SharedState
                _SyncPlayerActivityFromSnapshot -Prefix $key -Profile $Profile -SharedState $SharedState -Count ([int]$parsed.Count)
                Set-JoinableServerRuntimeState -Prefix $key -Force -Detail 'Minecraft server is joinable and responding to player queries.' -SharedState $SharedState
                try { _MaybePromoteJoinableFromCommandResponse -Prefix $key -Signal 'minecraft-rcon-list' -SharedState $SharedState } catch { }
                $pollState.Source = $usedSource
                $pollState.Note = $parsed.Note
                $pollState.LastSuccessAt = $now
                return $true
            }
            $pollState.Note = if ($r.Response) { 'Minecraft RCON responded, but no roster was parsed yet.' } else { 'Minecraft RCON responded with no player text.' }
        } else {
            $pollState.Note = if ($r.Error) { "$($r.Error)" } else { 'Minecraft RCON player query failed.' }
        }
    }

    $sent = Send-ServerStdin -Prefix $key -Command 'list'
    $usedSource = 'MinecraftListLog'
    if (-not $sent) {
        $pollState.Source = if ([string]::IsNullOrWhiteSpace($usedSource)) { 'MinecraftListLog' } else { $usedSource }
        if ([string]::IsNullOrWhiteSpace($pollState.Note)) {
            $pollState.Note = 'Could not send the Minecraft list command.'
        }
        return $false
    }

    Start-Sleep -Milliseconds 900
    $recentLogText = _ReadRecentProfileLogText -Profile $Profile -Prefix $key -TailLines 120
    $parsed = _ParseMinecraftPlayersText -Text $recentLogText
    $pollState.Source = $usedSource
    $pollState.Note = $parsed.Note
    if (-not $parsed.Available) { return $false }
    if (-not $SharedState.RunningServers.ContainsKey($key)) { return $false }
    ...
    _SyncPlayerActivityFromSnapshot -Prefix $key -Profile $Profile -SharedState $SharedState -Count ([int]$parsed.Count)
    Set-JoinableServerRuntimeState -Prefix $key -Force -Detail 'Minecraft server is joinable and responding to player queries.' -SharedState $SharedState
    try { _MaybePromoteJoinableFromCommandResponse -Prefix $key -Signal 'minecraft-stdin-list' -SharedState $SharedState } catch { }
    $pollState.LastSuccessAt = $now
    return $true
}
...
function _GetPlayerObservationSnapshot {
    ...
    if (_TestProfileGame -Profile $Profile -KnownGame '7DaysToDie') {
        ...
        try {
            $observedAt = $null
            if ($SharedState.ContainsKey('LatestPlayerObservedAt') -and $SharedState.LatestPlayerObservedAt -and $SharedState.LatestPlayerObservedAt.ContainsKey($key)) {
                $observedAt = [datetime]$SharedState.LatestPlayerObservedAt[$key]
            }
            $observedAgeSeconds = ''
            if ($observedAt) {
                try { $observedAgeSeconds = [int][Math]::Round(((Get-Date) - $observedAt).TotalSeconds) } catch { $observedAgeSeconds = '' }
            }
            _Trace7DaysToDieDebug -Prefix $key -Message ("snapshot supported={0} available={1} count={2} source={3} observedAgeSeconds={4} note={5}" -f `
                $snapshot.Supported, $snapshot.Available, $snapshot.Count, $(if ($snapshot.Source) { $snapshot.Source } else { '<none>' }), `
                $(if ($observedAgeSeconds -ne '') { $observedAgeSeconds } else { '<none>' }), $snapshot.Note)
        } catch { }
        _TracePlayerObservationDecision -Prefix $key -Profile $Profile -Snapshot $snapshot -SharedState $SharedState
        return $snapshot
    }
    ...
    if (_TestProfileGame -Profile $Profile -KnownGame 'Minecraft') {
        ...
        if ($SharedState.ContainsKey('LatestPlayerCounts') -and $SharedState.LatestPlayerCounts.ContainsKey($key)) {
            ...
            $snapshot.Available = $true
            $snapshot.Count = [Math]::Max(0, $playerCount)
            $snapshot.Source = 'MinecraftHeartbeat'
            $snapshot.Note = 'Player count observed from Minecraft list polling.'
        } else {
            $requested = $false
            try { $requested = (_RefreshMinecraftPlayers -Prefix $key -Profile $Profile -SharedState $SharedState) } catch { $requested = $false }
            if ($requested -and $SharedState.ContainsKey('LatestPlayerCounts') -and $SharedState.LatestPlayerCounts.ContainsKey($key)) {
                $playerCount = 0
                try { $playerCount = [int]$SharedState.LatestPlayerCounts[$key] } catch { $playerCount = 0 }
                $snapshot.Available = $true
                $snapshot.Count = [Math]::Max(0, $playerCount)
                ...
                $snapshot.Note = 'Waiting for Minecraft list polling.'
            }
        }
        _TracePlayerObservationDecision -Prefix $key -Profile $Profile -Snapshot $snapshot -SharedState $SharedState
        return $snapshot
    ...
    if (_TestProfileGame -Profile $Profile -KnownGame 'Satisfactory') {
        ...
        try {
            if ($SharedState.ContainsKey('LatestPlayers') -and $SharedState.LatestPlayers.ContainsKey($key)) {
                $cachedNames = @($SharedState.LatestPlayers[$key] | Where-Object { -not [string]::IsNullOrWhiteSpace([string]$_) })
            }
        } catch { $cachedNames = @() }
        try {
            if ($SharedState.ContainsKey('LatestPlayerObservedAt') -and $SharedState.LatestPlayerObservedAt.ContainsKey($key)) {
        ...
        try {
            ...
            if ($r.Success) {
                ...
                $useObservedRosterFallback = $false
                ...
                if ($apiCount -le 0 -and @($cachedNames).Count -gt 0) {
                    $useObservedRosterFallback = $true
                }

                $snapshot.Available = $true
                if ($useObservedRosterFallback) {
                    $snapshot.Count = [Math]::Max($cachedCount, @($cachedNames).Count)
                    $snapshot.Source = 'SatisfactoryLog'
                    $snapshot.Note = 'Player roster observed from Satisfactory join/leave logs while API reports zero.'
                } elseif ($useRecentLogFallback) {
...
function _CheckIdleShutdown {
    ...
    try {
        if (_TestProfileGame -Profile $Profile -KnownGame '7DaysToDie') {
            _Trace7DaysToDieDebug -Prefix $Prefix -Message ("idle-check runtime={0} supported={1} available={2} currentCount={3} pendingRule={4} firstPlayerSeen={5} lastBecameEmpty={6} dueAt={7} note={8}" -f `
                $(if ($runtimeCode) { $runtimeCode } else { '<none>' }), $activity.DetectionSupported, $activity.DetectionAvailable, `
                $activity.CurrentCount, $(if ($activity.PendingRule) { $activity.PendingRule } else { '<none>' }), `
                $(if ($activity.FirstPlayerSeenAt) { ([datetime]$activity.FirstPlayerSeenAt).ToString('o') } else { '<none>' }), `
                $(if ($activity.LastBecameEmptyAt) { ([datetime]$activity.LastBecameEmptyAt).ToString('o') } else { '<none>' }), `
                $(if ($activity.ShutdownDueAt) { ([datetime]$activity.ShutdownDueAt).ToString('o') } else { '<none>' }), `
                $(if ($activity.Note) { $activity.Note } else { '<none>' }))
        }
    } catch { }
    ...
    if ($startupNoPlayersMinutes -gt 0 -and $null -eq $activity.FirstPlayerSeenAt) {
        ...
        if ((Get-Date) -ge $dueAt) {
            ...
            try {
                Invoke-SafeShutdown -Prefix $key -Quiet -SkipInGameAnnouncements | Out-Null
            } catch {
...
function New-DiscordGameMessage {
    ...
    switch ($eventName.ToLowerInvariant()) {
        ...
        'autosave_started' {
            $templates = @(
                '[AUTOSAVE STARTED] {GameName} auto-save started. {SaveLead}',
                '[AUTOSAVE STARTED] {GameName} is locking in progress now. {SaveLead}',
                '[AUTOSAVE STARTED] ECC kicked off an auto-save for {GameName}. {SaveLead}',
                '[AUTOSAVE STARTED] {GameName} is writing down the latest chapter. {SaveLead}',
                '[AUTOSAVE STARTED] {GameName} is preserving the current run. {SaveLead}',
                '[AUTOSAVE STARTED] {GameName} is tucking the latest {WorldLabel} state away. {SaveLead}',
                '[AUTOSAVE STARTED] ECC is banking the latest progress for {GameName}. {SaveLead}',
                '[AUTOSAVE STARTED] {GameName} is snapping a safety save before the next round. {SaveLead}',
                '[AUTOSAVE STARTED] {GameName} is pinning down the newest state before anything stupid happens. {SaveLead}',
                '[AUTOSAVE STARTED] ECC just told {GameName} to bank the latest run before the next hit lands. {SaveLead}',
                '[AUTOSAVE STARTED] The {WorldLabel} in {GameName} is getting folded into a safety snapshot. {SaveLead}',
                '[AUTOSAVE STARTED] {GameName} is writing the fresh state to the shelf before anyone notices. {SaveLead}'
            )
        }
        'autosave_done' {
            $templates = @(
                '[AUTOSAVE COMPLETED] {GameName} auto-save completed successfully. {SaveLead}',
                '[AUTOSAVE COMPLETED] {GameName} progress was saved cleanly. {SaveLead}',
                '[AUTOSAVE COMPLETED] {GameName} finished its auto-save without a hitch. {SaveLead}',
                '[AUTOSAVE COMPLETED] {GameName} tucked the latest progress away safely. {SaveLead}',
                '[AUTOSAVE COMPLETED] {GameName} preserved the latest state cleanly. {SaveLead}',
                '[AUTOSAVE COMPLETED] {GameName} banked the latest {WorldLabel} snapshot without a hitch. {SaveLead}',
                '[AUTOSAVE COMPLETED] ECC locked down the newest progress for {GameName}. {SaveLead}',
                '[AUTOSAVE COMPLETED] {GameName} wrapped the safety save cleanly. {SaveLead}',
                '[AUTOSAVE COMPLETED] {GameName} sealed the newest chapter without dropping a page. {SaveLead}',
                '[AUTOSAVE COMPLETED] ECC got the latest {WorldLabel} state tucked away cleanly for {GameName}. {SaveLead}',
                '[AUTOSAVE COMPLETED] The newest run in {GameName} is safely on the shelf. {SaveLead}',
                '[AUTOSAVE COMPLETED] {GameName} put the fresh snapshot away and never missed a beat. {SaveLead}'
            )
        }
        'blocked' {
            $templates = @(
...
function Update-HytaleServerFiles {
    ...
    if ($status -and $status.Running) {
        ...
        try {
            Invoke-SafeShutdown -Prefix $Prefix -Quiet -SkipInGameAnnouncements | Out-Null
...
function Get-ServerStatus {
    ...
    if ($null -eq $proc -or $proc.HasExited) {
        $gameProc = _FindLiveProcessForProfile -Profile $profile
        if ($null -ne $gameProc -and -not $gameProc.HasExited) {
            $effectiveStartTime = $entry.StartTime
            try {
                $gameProcStartTime = $gameProc.StartTime
                if ($null -eq $effectiveStartTime -or [datetime]$gameProcStartTime -gt [datetime]$effectiveStartTime) {
                    # When we rebind to a newer real server process, prefer its actual
                    # start time so uptime resets after genuine restarts instead of
                    # inheriting the previous session's timer forever.
                    $effectiveStartTime = [datetime]$gameProcStartTime
                }
            } catch { }

            if ($null -eq $effectiveStartTime) {
                $effectiveStartTime = Get-Date
            }
            ...
            return @{
                ...
                Pid       = $gameProc.Id
                StartTime = $effectiveStartTime
                Uptime    = (Get-Date) - $effectiveStartTime
            }
        }
...
function Invoke-SafeShutdown {
    param([string]$Prefix, [switch]$Quiet, [switch]$SkipInGameAnnouncements)
    ...
    if (-not $SkipInGameAnnouncements) {
        try {
            [void](_InvokeInGameAnnouncementCountdown -Prefix $prefix -Profile $profile -Kind 'shutdown' -SharedState $script:State)
        } catch {
            _Log "[$($profile.GameName)] Shutdown countdown announcement failed: $($_.Exception.Message)" -Level WARN
        }
    }
...
function Restart-GameServer {
    ...
    try {
        [void](_InvokeInGameAnnouncementCountdown -Prefix $prefix -Profile $profile -Kind 'restart' -SharedState $script:State)
    } catch {
        _Log "[$($profile.GameName)] Restart countdown announcement failed: $($_.Exception.Message)" -Level WARN
    }

    Invoke-SafeShutdown -Prefix $prefix -Quiet -SkipInGameAnnouncements
...
function Send-ServerStdin {
    ...
    if (-not $preferWindow -and $script:State.StdinHandles.ContainsKey($prefix)) {
        try {
            ...
            try { $script:State.StdinHandles.Remove($prefix) | Out-Null } catch { }
            $detail.Error = "$_"
        }
    }
...
function _ExecuteStop {
    ...
    switch ($stopMethod) {
        'stdin' {
            ...
            $stdinSent = $false
            try {
                $stdinSent = [bool](Send-ServerStdin -Prefix $prefix -Command $Profile.StdinStopCommand)
            } catch {
                $stdinSent = $false
            }
            if (-not $stdinSent) {
                _Log "[$($Profile.GameName)] Graceful stdin stop command could not be delivered after reload/rehydration. Falling back to forced stop if the process stays alive." -Level WARN
            }
            $proc = $null
            try {
                $proc = Get-Process -Id $entry.Pid -ErrorAction Stop
        ...
        'processName' {
            ...
            # Also kill the launcher wrapper PID just in case, but only when the
            # tracked PID still looks like it belongs to this profile. A stale
            # rehydrated entry must never let one profile kill another server.
            $shouldKillTrackedPid = $false
            try {
                $trackedProc = Get-Process -Id $entry.Pid -ErrorAction Stop
                $trackedName = ''
                try { $trackedName = $trackedProc.ProcessName } catch { $trackedName = '' }

                if ($procName -and $trackedName -ieq $procName) {
                    $shouldKillTrackedPid = $true
                } elseif ($trackedName -in @('cmd','powershell','pwsh')) {
                    $shouldKillTrackedPid = $true
                } else {
                    $folderPath = ''
                    $procPath = ''
                    $cmdLine = ''
                    try { $folderPath = [Environment]::ExpandEnvironmentVariables((_CoalesceStr $Profile.FolderPath '')) } catch { $folderPath = (_CoalesceStr $Profile.FolderPath '') }
                    try { $procPath = "$($trackedProc.Path)" } catch { $procPath = '' }
                    try {
                        $cim = Get-CimInstance Win32_Process -Filter "ProcessId = $($trackedProc.Id)" -ErrorAction Stop
                        if ($cim) {
                            if (-not $procPath) { $procPath = "$($cim.ExecutablePath)" }
                            $cmdLine = "$($cim.CommandLine)"
                        }
            ...
            } catch { $shouldKillTrackedPid = $false }

            if ($shouldKillTrackedPid) {
                _KillProcessTree -RootPid $entry.Pid
            } else {
                _Log "[$($Profile.GameName)] Skipped killing tracked PID $($entry.Pid): it does not match this profile." -Level WARN
            }
        }
        'http' {
...
function _CheckAutoSave {
    ...
    try {
        if (_ShouldSendInGameAnnouncement -Prefix $Prefix -Profile $Profile -SharedState $SharedState) {
            [void](_SendInGameAnnouncement -Prefix $Prefix -Profile $Profile -Kind 'save')
        }
    } catch {
        _Log "[$($Profile.GameName)] Auto-save announcement failed: $($_.Exception.Message)" -Level WARN
    }
...
function _CheckScheduledRestart {
    ...
    try {
        [void](_InvokeInGameAnnouncementCountdown -Prefix $Prefix -Profile $Profile -Kind 'restart' -SharedState $SharedState)
    } catch {
        _Log "[$($Profile.GameName)] Scheduled restart countdown announcement failed: $($_.Exception.Message)" -Level WARN
    }
    ...
    while (-not ($SharedState.ContainsKey('StopMonitor') -and $SharedState['StopMonitor'] -eq $true)) {
        ...
        foreach ($prefix in @($SharedState.Profiles.Keys)) {
            ...
            try {
                ...
                try {
                    if ($SharedState.RunningServers.ContainsKey($prefix) -and (_TestProfileGame -Profile $profile -KnownGame 'Minecraft')) {
                        [void](_RefreshMinecraftPlayers -Prefix $prefix -Profile $profile -SharedState $SharedState)
                    }
                } catch {
                    _LogProfileError -Prefix $prefix -Profile $profile -Action 'refresh-players' -Function '_RefreshMinecraftPlayers' -Message 'Minecraft player refresh failed.' -ErrorRecord $_ -Fallback 'ECC kept the last trusted Minecraft player snapshot and will retry the refresh on the next monitor tick.' -Recovery 'degraded but still running; player refresh retry pending next tick'
                }
...
function Invoke-RconCommand {
    ...
    function _ReadInt32 {
        ...
        if ($bytes.Length -lt 4) {
            if ($bytes.Length -eq 0) {
                throw [System.IO.EndOfStreamException]::new("Connection closed unexpectedly")
            }
    ...
    try {
        ...
        $receivedResponsePacket = $false
        while ((Get-Date) -lt $deadline) {
            try {
                $resp = _ReadPacket $reader
                if ($resp.Id -eq 2) {
                    $receivedResponsePacket = $true
                ...
                if (($_.Exception -is [System.IO.EndOfStreamException]) -and $receivedResponsePacket) {
                    if ($CaptureDebug) { $dbg.Add("response stream closed after command output") | Out-Null }
                    break
                }
                break
            }
...
function Invoke-ServerCommandText {
    ...
    # Minecraft console commands behave more reliably over stdin than ad-hoc RCON.
    # Keep explicit RCON command definitions intact, but prefer stdin for freeform UI text.
    if (_TestProfileGame -Profile $profile -KnownGame 'Minecraft') {
        if ($VerboseDebug) {
            $debug.Add("transport: STDIN (minecraft preferred)") | Out-Null
            $debug.Add("cmd: $cmd") | Out-Null
        }
...
function Invoke-ProfileCommand {
    ...
    if ($CommandName -eq 'save' -and $cmdType -ne 'Status') {
        try {
            if (_ShouldSendInGameAnnouncement -Prefix $prefix -Profile $profile -SharedState $state) {
                [void](_SendInGameAnnouncement -Prefix $prefix -Profile $profile -Kind 'save')
            }
        } catch {
            _Log "[$($profile.GameName)] Manual save announcement failed: $($_.Exception.Message)" -Level WARN
        }

        $result = _ExecuteSave -Prefix $prefix -Profile $profile
        $resultMsg = if ($result) {
            ...
            New-DiscordGameMessage -Profile $profile -Prefix $prefix -Event 'command_error' -Values @{
                Command = "'save'"
                Reason  = 'The save command could not be executed for this server.'
            }
        }
    ...
    switch ($cmdType) {
        ...
        'Status' {
            $saveMethodLower = ''
            try { $saveMethodLower = "$($profile.SaveMethod)".Trim().ToLowerInvariant() } catch { $saveMethodLower = '' }

            if ($CommandName -eq 'save' -and $saveMethodLower -eq 'none') {
                $result = $true
                $resultMsg = "[SAVE] $($profile.GameName) - no manual save command is available for this server. World saving remains automatic."
            } elseif ($CommandName -eq 'players' -and (_TestProfileGame -Profile $profile -KnownGame 'Valheim')) {
                $result = $true
                $resultMsg = "[PLAYERS] $($profile.GameName) - live player listing is not supported yet for this server."
            } else {
                $status    = Get-ServerStatus -Prefix $prefix
                $result    = $true
        ...
        'Rcon' {
            ...
            if (-not $rconPass) {
                ...
                if ($rconResult.Success) {
                    ...
                    $parsedPlayers = $null
                    if ((_TestProfileGame -Profile $profile -KnownGame 'Minecraft') -and ($CommandName -eq 'players' -or $rconCmd -eq 'list')) {
                        try {
                            $parsedPlayers = _ParseMinecraftPlayersText -Text $resp
                            if ($parsedPlayers.Available) {
                                _SetLatestPlayersSnapshot -Prefix $prefix -Names @($parsedPlayers.Names) -Count ([int]$parsedPlayers.Count) -SharedState $state
                                _SyncPlayerActivityFromSnapshot -Prefix $prefix -Profile $profile -SharedState $state -Count ([int]$parsedPlayers.Count)
                                if ($state -and $state.ContainsKey('PlayerQueryState')) {
                                    $state.PlayerQueryState[$prefix] = [ordered]@{
                                        LastAttemptAt = Get-Date
                                        LastSuccessAt = Get-Date
                                        Source        = 'MinecraftRcon'
                                        Note          = $parsedPlayers.Note
                                    }
                                }
                            }
                            Set-JoinableServerRuntimeState -Prefix $prefix -Force -SharedState $state
                        } catch { }
                    }
                    $resultMsg = if ((_TestProfileGame -Profile $profile -KnownGame 'Minecraft') -and ($CommandName -eq 'players' -or $rconCmd -eq 'list') -and $parsedPlayers -and $parsedPlayers.Available) {
                        if ([int]$parsedPlayers.Count -le 0) {
                            "[PLAYERS] $($profile.GameName) - no players online."
                        } elseif (@($parsedPlayers.Names).Count -gt 0) {
                            "[PLAYERS] $($profile.GameName) - online: $((@($parsedPlayers.Names)) -join ', ')"
                        ...
                        "[RCON] $($profile.GameName): $resp"
                    } else {
                        "[RCON] $($profile.GameName): command sent (no response)"
                    }
                } else {
        ...
        'Rest' {
            ...
            if (-not $endpoint) {
                ...
                if ($restResult) {
                    $result    = $true
                    if ((_TestProfileGame -Profile $profile -KnownGame 'Palworld') -and $CommandName -eq 'save') {
                        $resultMsg = "[SAVE] $($profile.GameName) - world saved successfully."
                    } elseif ((_TestProfileGame -Profile $profile -KnownGame 'Palworld') -and $CommandName -eq 'players') {
                        $playerNames = @()
                        try {
                            if ($restResult -and $restResult.PSObject.Properties['players']) {
                                foreach ($entry in @($restResult.players)) {
                                    $name = $null
                                    foreach ($candidate in @('name','playerName','nickname','steamName','username')) {
                                        try {
                                            if ($entry -and $entry.PSObject.Properties[$candidate] -and -not [string]::IsNullOrWhiteSpace("$($entry.$candidate)")) {
                                                $name = "$($entry.$candidate)".Trim()
                                                break
                                            }
                                        } catch { }
                                    }
                                    if (-not [string]::IsNullOrWhiteSpace($name)) { $playerNames += $name }
                                }
                                if (@($playerNames).Count -le 0) {
                                    $playerCount = @($restResult.players).Count
                                    if ($playerCount -le 0) {
                                        $resultMsg = "[PLAYERS] $($profile.GameName) - no players online."
                                    } else {
                                        $resultMsg = "[PLAYERS] $($profile.GameName) - players online: $playerCount"
                                    }
                                } else {
                                    $resultMsg = "[PLAYERS] $($profile.GameName) - online: $($playerNames -join ', ')"
                                }
                            } elseif ($restResult -is [System.Collections.IEnumerable] -and -not ($restResult -is [string])) {
                                $playerCount = @($restResult).Count
                                if ($playerCount -le 0) {
                                    $resultMsg = "[PLAYERS] $($profile.GameName) - no players online."
                                } else {
                                    $resultMsg = "[PLAYERS] $($profile.GameName) - players online: $playerCount"
                                ...
                                $resultMsg = "[PLAYERS] $($profile.GameName) - player query completed."
                            }
                        } catch {
                            $resultMsg = "[PLAYERS] $($profile.GameName) - player query completed."
                        }
                    } elseif ((_TestProfileGame -Profile $profile -KnownGame 'Palworld') -and ($CommandName -in @('status','info'))) {
                        $playerCount = $null
                        foreach ($candidate in @('players','numPlayers','playerCount','currentPlayers')) {
                            try {
                                if ($restResult -and $restResult.PSObject.Properties[$candidate]) {
                                    $value = $restResult.$candidate
                                    if ($value -is [System.Collections.IEnumerable] -and -not ($value -is [string])) {
                                        $playerCount = @($value).Count
                                    } elseif ($null -ne $value -and "$value" -match '^\d+$') {
                                        $playerCount = [int]$value
                                    }
                                    if ($null -ne $playerCount) { break }
                                }
                            } catch { }
                        }
                        if ($null -ne $playerCount) {
                            $resultMsg = "[STATUS] $($profile.GameName) - players online: $playerCount"
                        } else {
                            $resultMsg = "[STATUS] $($profile.GameName) - REST query completed."
                        }
                    } else {
