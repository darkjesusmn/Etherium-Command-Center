...
function Initialize-Logging {
    ...
    $script:BreadcrumbFile = Join-Path $LogDir 'crash-breadcrumbs.log'
    try {
        if (-not (Test-Path $script:BreadcrumbFile)) {
            [System.IO.File]::WriteAllText($script:BreadcrumbFile, '')
        }
    } catch { }
    Write-Log "Logging initialised -> $($script:LogFile)" -Level INFO
    Write-CrashBreadcrumb -Message 'Logging session initialised.' -Category 'Logging'
}
...
function Get-CrashBreadcrumbPath {
    return $script:BreadcrumbFile
}
...
function Write-CrashBreadcrumb {
    param(
        [string]$Message,
    ...
    $target = $script:BreadcrumbFile
    if ([string]::IsNullOrWhiteSpace($target)) {
        $target = Join-Path (Join-Path (Get-Location) 'Logs') 'crash-breadcrumbs.log'
    }
    ...
    try {
        ...
        $entry = "[{0}][{1}][{2}] {3}{4}" -f `
            ...
            $Message.Trim(), `
            [Environment]::NewLine
        [System.IO.File]::AppendAllText($target, $entry, [System.Text.Encoding]::UTF8)
    } catch {
        _WriteLoggingFallbackWarning -Kind 'Write' -Message "Failed to write crash breadcrumb to '$target': $($_.Exception.Message)" -ThrottleSeconds 15
    }
}
...
Export-ModuleMember -Function Initialize-Logging, Get-LogFilePath, Get-CrashBreadcrumbPath, Set-DebugLoggingEnabled, Write-CrashBreadcrumb, Write-Log
