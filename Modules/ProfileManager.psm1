...
$script:KnownGames = @{
    ...
    # -------------------------------------------------------------------------
    #  MINECRAFT  (Vanilla / Paper / Purpur / Spigot / Forge / NeoForge / Fabric)
    #  - Usually launched as a JAR directly via java.exe.
    #  - ECC can also launch a profile with Executable = java or java.exe when
    #    the pack expects a direct Java command line instead of a jar target.
...
function _TryGetMinecraftJavaLaunchFromBatch {
    ...
    foreach ($candidateName in @('run.bat','start-server.bat','start.bat')) {
        $candidatePath = Join-Path $FolderPath $candidateName
        if (-not (Test-Path -LiteralPath $candidatePath)) { continue }

        $raw = ''
        try { $raw = Get-Content -LiteralPath $candidatePath -Raw -ErrorAction Stop } catch { $raw = '' }
        ...
        foreach ($lineRaw in $lines) {
            ...
            if ($line -notmatch '^(?i:java(?:\.exe)?)\b') { continue }

            $launchArgs = ($line -replace '^(?i:java(?:\.exe)?)\s*', '').Trim()
            if ([string]::IsNullOrWhiteSpace($launchArgs)) { continue }

            $launchArgs = ($launchArgs -replace '\s+%[\*\d]+\s*$', '').Trim()
            if ([string]::IsNullOrWhiteSpace($launchArgs)) { continue }

            $result.Found = $true
            $result.Executable = 'java'
            $result.LaunchArgs = $launchArgs
            $result.SourceFile = $candidateName
            return [pscustomobject]$result
        }
    }
...
function _EnsureMinecraftNoGuiArg {
    param([string]$LaunchArgs)

    $trimmed = if ($null -ne $LaunchArgs) { "$LaunchArgs".Trim() } else { '' }
    if ([string]::IsNullOrWhiteSpace($trimmed)) { return 'nogui' }
    if ($trimmed -match '(^|\s)nogui(\s|$)') { return $trimmed }
    return ($trimmed + ' nogui').Trim()
}

function Resolve-MinecraftLaunchProfile {
    param(
        [string]$FolderPath,
        [string]$DetectedExecutable = '',
        [string]$CurrentLaunchArgs = ''
    )

    $result = [ordered]@{
        LaunchType       = 'simple-jar'
        Executable       = if ([string]::IsNullOrWhiteSpace($DetectedExecutable)) { 'server.jar' } else { $DetectedExecutable }
        LaunchArgs       = _EnsureMinecraftNoGuiArg -LaunchArgs $CurrentLaunchArgs
        SourceFile       = ''
        Detail           = 'Using standard Minecraft jar launch.'
    }
    ...
    $batchLaunch = _TryGetMinecraftJavaLaunchFromBatch -FolderPath $FolderPath
    $hasVariables = Test-Path -LiteralPath (Join-Path $FolderPath 'variables.txt')
    $hasJvmArgs = Test-Path -LiteralPath (Join-Path $FolderPath 'user_jvm_args.txt')
    $hasWinArgs = @(Get-ChildItem -LiteralPath $FolderPath -Recurse -File -Filter 'win_args.txt' -ErrorAction SilentlyContinue | Select-Object -First 1).Count -gt 0

    if ($batchLaunch.Found -and ($hasVariables -or $hasJvmArgs -or $hasWinArgs)) {
        $result.LaunchType = 'pack-wrapper'
        $result.Executable = $batchLaunch.Executable
        $result.LaunchArgs = _EnsureMinecraftNoGuiArg -LaunchArgs $batchLaunch.LaunchArgs
        $result.SourceFile = $batchLaunch.SourceFile
        $result.Detail = "Resolved pack-managed Java launch from $($batchLaunch.SourceFile)."
        return [pscustomobject]$result
    }

    $exeName = ''
    try { $exeName = [System.IO.Path]::GetFileName([string]$DetectedExecutable) } catch { $exeName = [string]$DetectedExecutable }
    if ($exeName -and $exeName.Equals('java.exe', [System.StringComparison]::OrdinalIgnoreCase)) {
        $result.LaunchType = 'direct-java'
        $result.Executable = 'java'
        $result.LaunchArgs = _EnsureMinecraftNoGuiArg -LaunchArgs $CurrentLaunchArgs
        $result.Detail = 'Using direct Java executable launch.'
        return [pscustomobject]$result
    }

    $result.LaunchType = 'simple-jar'
    $result.Executable = if ([string]::IsNullOrWhiteSpace($DetectedExecutable)) { 'server.jar' } else { $DetectedExecutable }
    $result.LaunchArgs = _EnsureMinecraftNoGuiArg -LaunchArgs $CurrentLaunchArgs
    $result.Detail = 'Using standard jar launch.'
    return [pscustomobject]$result
}
...
function New-GameProfile {
    ...
    $minecraftLaunchProfile = $null
    if ($knownKey -eq 'Minecraft') {
        $minecraftLaunchProfile = Resolve-MinecraftLaunchProfile -FolderPath $FolderPath -DetectedExecutable $Executable -CurrentLaunchArgs $LaunchArgs
        if ($minecraftLaunchProfile) {
            $Executable = [string]$minecraftLaunchProfile.Executable
            $resolvedExecutable = $Executable
            $LaunchArgs = [string]$minecraftLaunchProfile.LaunchArgs
            $explicitLaunchArgs = $LaunchArgs
            $preserveLaunchArgs = $true
            _PLog "Minecraft launch resolved: type='$($minecraftLaunchProfile.LaunchType)' source='$($minecraftLaunchProfile.SourceFile)' executable='$($minecraftLaunchProfile.Executable)' args='$($minecraftLaunchProfile.LaunchArgs)'" -Level DEBUG
        }
    }
    ...
    $profile = [ordered]@{
        ...
        Commands            = $commands
        MinecraftLaunchType = if ($minecraftLaunchProfile) { "$($minecraftLaunchProfile.LaunchType)" } else { '' }
        MinecraftLaunchSource = if ($minecraftLaunchProfile) { "$($minecraftLaunchProfile.SourceFile)" } else { '' }
        MinecraftLaunchDetail = if ($minecraftLaunchProfile) { "$($minecraftLaunchProfile.Detail)" } else { '' }
