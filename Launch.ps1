...
try {
    ...
    Write-Log "Root: $Root"
    Write-CrashBreadcrumb -Category 'Launch' -Message ("ECC launch starting. Root={0}" -f $Root)
    try {
        $recentHostCrash = Get-WinEvent -FilterHashtable @{
            LogName   = 'Application'
            ...
            Where-Object {
                ...
                ($_.Message -match 'clr\.dll|80131506|APPCRASH|internal error in the \.NET Runtime')
            } |
            Sort-Object TimeCreated -Descending |
            Select-Object -First 1

        if ($recentHostCrash) {
            $eventSummary = ($recentHostCrash.Message -split "`r?`n" |
                Where-Object { $_ -match 'Faulting application name|Faulting module name|Exception code|Fault offset|exit code|Event Name|Report Id' } |
                ForEach-Object { $_.Trim() }) -join ' | '
            Write-Log ("Recent PowerShell/.NET host crash found at {0}: {1}" -f $recentHostCrash.TimeCreated, $eventSummary) -Level WARN
            Write-CrashBreadcrumb -Category 'WER' -Level 'WARN' -Message ("Recent PowerShell/.NET host crash found at {0}: {1}" -f $recentHostCrash.TimeCreated, $eventSummary)
        }
    } catch {
        Write-Log "Could not inspect Windows crash history: $($_.Exception.Message)" -Level DEBUG
    }
    ...
    $SharedState['CrashBreadcrumbPath'] = Get-CrashBreadcrumbPath

    if (-not $SharedState.ContainsKey('GlobalExceptionHooksInstalled')) {
        $SharedState['GlobalExceptionHooksInstalled'] = $false
    }
    if (-not [bool]$SharedState['GlobalExceptionHooksInstalled']) {
        try {
            $domainHandler = [System.UnhandledExceptionEventHandler]{
                param($sender, $args)
                try {
                    ...
                    $msg = if ($ex -is [System.Exception]) {
                        "{0} | terminating={1} | stack={2}" -f $ex.ToString(), $isTerminating, $ex.StackTrace
                    } else {
                        "{0} | terminating={1}" -f [string]$ex, $isTerminating
                    }
                    Write-CrashBreadcrumb -Category 'AppDomain' -Level 'ERROR' -Message $msg
                } catch { }
            }
            [System.AppDomain]::CurrentDomain.add_UnhandledException($domainHandler)
            $SharedState['GlobalUnhandledExceptionHandler'] = $domainHandler
        } catch {
            Write-CrashBreadcrumb -Category 'Launch' -Level 'WARN' -Message ("Failed to register AppDomain hook: {0}" -f $_.Exception.Message)
        }
        ...
        try {
            $taskHandler = [System.EventHandler[System.Threading.Tasks.UnobservedTaskExceptionEventArgs]]{
                ...
                try {
                    $agg = $args.Exception
                    $msg = if ($agg) { $agg.ToString() } else { 'Unobserved task exception with no aggregate exception payload.' }
                    Write-CrashBreadcrumb -Category 'TaskScheduler' -Level 'ERROR' -Message $msg
                    if ($args) { $args.SetObserved() }
                } catch { }
            }
            [System.Threading.Tasks.TaskScheduler]::add_UnobservedTaskException($taskHandler)
            $SharedState['GlobalTaskExceptionHandler'] = $taskHandler
        } catch {
            Write-CrashBreadcrumb -Category 'Launch' -Level 'WARN' -Message ("Failed to register task hook: {0}" -f $_.Exception.Message)
        }

        $SharedState['GlobalExceptionHooksInstalled'] = $true
        Write-CrashBreadcrumb -Category 'Launch' -Message 'Registered global exception hooks.'
    }
    ...
    Write-CrashBreadcrumb -Category 'Runspace' -Message 'Discord listener runspace started.'
    ...
    Write-CrashBreadcrumb -Category 'Runspace' -Message 'Server monitor runspace started.'
    $SharedState['MonitorRunspace'] = $monitorRunspace
    $SharedState['MonitorPS']       = $monitorPS
    $SharedState['MonitorHandle']   = $monitorHandle
    ...
    do {
        ...
        try {
            ...
            Write-CrashBreadcrumb -Category 'Launch' -Message 'Entering Start-GUI.'
Start-GUI -SharedState $SharedState -ConfigPath $ConfigPath -ProfilesDir $ProfilesDir -TranscriptPath $TranscriptPath
            Write-CrashBreadcrumb -Category 'Launch' -Message 'Start-GUI returned to launch script.'
        } catch {
            $stage = if ($pendingUiReloadRelaunch) { 'UI reload relaunch' } else { 'GUI launch' }
            Write-Log "Fatal failure during ${stage}: $_" -Level ERROR
            Write-CrashBreadcrumb -Category 'Launch' -Level 'ERROR' -Message ("Fatal failure during {0}: {1}" -f $stage, $_.Exception.ToString())
    ...
    Write-CrashBreadcrumb -Category 'Launch' -Message 'GUI closed; shutdown path starting.'

    $managedShutdownCompleted = $false
    try {
        if ($SharedState.ContainsKey('ManagedShutdownComplete')) {
            $managedShutdownCompleted = [bool]$SharedState['ManagedShutdownComplete']
        }
    } catch { $managedShutdownCompleted = $false }

    $stopServersBeforeExit = (-not $managedShutdownCompleted) -and ($fatalGuiReloadFailure -eq $true -or $SharedState.RestartProgram -eq $true)
    ...
    Write-CrashBreadcrumb -Category 'Launch' -Message 'ECC stopped cleanly.'
    ...
    try {
        Write-CrashBreadcrumb -Category 'Launch' -Level 'ERROR' -Message ("Top-level launch failure: {0}" -f $_.Exception.ToString())
    } catch { }
    Write-Host ""
