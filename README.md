...
- A polished Windows desktop UI with ECC-styled editors, dialogs, and add-game flow
- Start, stop, restart, status, and command routing per server profile
- Bulk top-bar actions for `Start All`, `Stop All`, `Reload UI`, `Reload Bot`, `Reload Commands`, `Full Restart`, and `Settings`
...
- In-game save, shutdown, and restart announcements for supported games that expose a player-facing announce command
- A config editor for common server config file types
- A generated/raw config editing view for supported config files
- Live Program Log, Discord log, and per-game log tabs
- Auto-save, scheduled restart, and auto-restart crash recovery
- Per-profile health visibility, player-source visibility, and timer/status summaries on dashboard cards
- Launch argument editors for supported games
- Performance Trace Mode for long-session lag investigation without full debug spam
...
If you just want to get ECC running fast:
...
2. Open `Settings` and fill in:
   ...
   - `MonitorChannelId`
   - `WebhookUrl`
   if you want Discord control and notifications.
3. Click `+ Add Game`.
4. Choose the game type.
5. Pick the real server folder in the ECC folder picker.
6. Open the new profile and verify the important fields before trusting it.
7. Start the server once from the dashboard.
8. Confirm:
   - logs appear
   - save works
   - stop works
   - player/status behavior looks correct

If any of those fail, treat the profile as a starting point and fix it before using ECC as the normal control path.
...
| Minecraft | `MC` | stdin + RCON | Java/JAR launch with `latest.log` tailing, stdin save/stop, and RCON player queries |
...
- Total trusted player count across visible server cards
- Bot state

It also includes these actions:

- `Start All`: starts every profile ECC currently considers ready to launch
- `Stop All`: stops every running profile using its configured shutdown path
...
`DISK` and `PLAYERS` also expose tooltip breakdowns in the live UI so operators can see where the totals are coming from.
...
1. ECC asks you to choose the game type first
2. ECC opens an ECC-styled folder picker for the server folder
3. ECC asks for a display name when the game is not already known
4. ECC can optionally ask for a separate config folder for manual/custom paths
5. ECC tries to match the game to a known template
6. ECC writes a new JSON profile into `Profiles`
...
- Runtime state badge and health badge
- Subtitle and source/player-observation line
...
| `EnablePerformanceDebugMode` | Enables long-run performance tracing without requiring full debug logging |
...
- Performance Trace Mode applies immediately and keeps normal logs quieter than full debug mode
...
ECC can also send in-game countdown/notice messages for supported profiles during planned save, stop, and restart flows:

- Minecraft via `say`
- 7 Days to Die via `say`
- Palworld via `broadcast`
- Project Zomboid via `servermsg`

ECC does not currently provide the same in-game announcement path for:
...
### Quick verification checklist for any new profile

Before you trust a newly created profile, verify these first:

1. `Executable` points to the real server start target.
2. `FolderPath` points to the real server install folder.
3. `LaunchArgs` still match how this server is meant to run on your machine.
4. `SaveMethod`, `StopMethod`, `StdinSaveCommand`, and `StdinStopCommand` make sense for that game.
5. Any required remote/admin path is filled in and tested:
   - `RCON`
   - `Telnet`
   - `REST`
   - `Satisfactory API`
6. The log path or log strategy is correct and live logs actually appear in ECC.
7. `EnableAutoRestart` is set the way you want before leaving the profile alone.
8. Start the server once from ECC and confirm:
   - the dashboard state looks right
   - logs appear
   - save works if supported
   - stop works cleanly
   - player detection works if the game supports it

If any of those fail, treat the profile as a starting point and fix it before relying on it for normal hosting.

### How ECC creates profiles

When you use **+ Add Game**, ECC:

1. Lets you choose the game type first
2. Opens the ECC folder picker so you can choose the server folder
3. Reads the selected folder name
4. Tries to match it to a known game template
5. Auto-generates a prefix
6. Searches the folder for a likely server executable
7. Builds base commands (`start`, `stop`, `restart`, `status`)
8. Adds template-specific commands like `save` and `players`
9. Saves the profile as JSON
...
3. Choose the server install folder in the ECC folder picker
4. If ECC recognizes the game, it builds the profile from the matching template
5. If ECC does not fully recognize the game, it asks for a display name
6. For unknown games, ECC can also ask whether you want to choose a config folder now
7. ECC saves the new profile and immediately opens it in the Profile Editor so you can review it
...
- You must verify manually:
  ...
  - `TelnetPort`
  - `TelnetPassword`
  - `LaunchArgs` still match your actual server config and ports
  - `EnableAutoRestart` is set the way you want
- Recommended checks/toggles:
  - `EnableAutoRestart`: usually `On`
  - `DisableFileTail`: usually `Off`
  - `CaptureOutput`: usually `Off` unless the log path is unreliable
- Do not trust the profile yet if:
  - `Test Telnet` fails
  - the server log never appears
  - `players` or `version` commands do not respond
...
- Usually auto-filled by ECC:
  ...
  - `BackupDir`
- You must verify manually:
  - Java is actually installed and reachable on `PATH`
  - `AOTCache` points to the correct cache location
  - `AssetFile` points to the correct downloaded assets package if used
  - `BackupDir` is valid and writable
  - `LaunchArgs` still reflect your intended server mode
- Recommended checks/toggles:
  - `EnableAutoRestart`: usually `On`
  - `DisableFileTail`: usually `Off`
  - `CaptureOutput`: only if your live log path is not useful
- Do not trust the profile yet if:
  - the server cannot start from ECC
  - Hytale updater/downloader tools cannot find the expected files
  - the live log never updates after launch
...
- Shows a `Hytale Tools` window with downloader and update actions
...
#### Hytale setup from an empty folder

ECC can help you build a Hytale server from an empty folder.

How to do it:

1. Make a new empty folder where you want the Hytale server to live.
2. Create a new Hytale profile in ECC and point it to that folder.
3. Open `Hytale Tools`.
4. Click `Update Downloader` to install the official Hytale downloader into that folder.
5. Click `Update Server` to download the newest Hytale server files.
6. ECC will unpack the new files and merge them into your Hytale folder.
7. After that, start the Hytale server from ECC.

Important notes:

- The folder can be empty at the start.
- The profile still needs to be made as a Hytale profile, not a generic profile.
- ECC uses the official Hytale downloader for version checks and server downloads.
- If the server is already running, ECC can stop it during the update.
- If `Auto-restart after update` is turned on, ECC can start it again when the update is done.
...
- Usually auto-filled by ECC:
  ...
  - `StopMethod`
- You must verify manually:
  - `LaunchArgs` really expose the REST API port
  - the REST API host/port match the actual running server
  - `AdminPassword` / `RESTAPIKey` are available through config or profile fields
  - `RconPort` is correct if you keep it as fallback
- Recommended checks/toggles:
  - `RestEnabled`: `On`
  - `RestPollOnlyWhenRunning`: usually `On`
  - `EnableAutoRestart`: usually `On`
  - `DisableFileTail`: often `On` if you prefer activity-feed mode
- Do not trust the profile yet if:
  - `Test REST` fails
  - `status`, `players`, or `save` do not work from ECC
  - the dashboard never marks the server joinable after REST comes online
...
This profile is for local Windows-hosted Java servers such as vanilla, Paper, Purpur, Spigot, Forge, NeoForge, and Fabric.
...
- Usually auto-filled by ECC:
  ...
  - `ConfigRoot`
- You must verify manually:
  - the chosen jar is the real startup jar for this pack
  - `MinRamGB` and `MaxRamGB` fit your machine
  - `RconHost`, `RconPort`, and `RconPassword` match `server.properties`
  - `LaunchArgs` are correct if the pack uses a custom Java launch line
  - `EnableAutoRestart` is set the way you want
- Recommended checks/toggles:
  - `EnableAutoRestart`: usually `On`
  - `DisableFileTail`: `Off`
  - `CaptureOutput`: `Off` unless the server does not keep a useful `latest.log`
- Do not trust the profile yet if:
  - the jar launches outside ECC but not inside ECC
  - `players` never work when RCON is expected to work
  - `logs\\latest.log` never updates
  - save/stop commands do not work cleanly from the dashboard

Important fields:

- `Executable`: usually `server.jar` or your server jar filename
...
- `ConfigRoot`: usually the server folder itself
- `ServerLogPath`: usually `logs\latest.log`

How ECC handles it:

- Launches the selected `.jar` through Java with the profile memory settings
- Can also launch direct `java` / `java.exe` profiles when a server pack needs a custom Java command line instead of a single jar target
- Tails `logs\latest.log`
- Marks the server joinable when the startup log reaches the normal `For help, type` ready line
- Uses stdin for clean save and stop behavior
- Uses RCON for the `players` command when RCON is enabled in `server.properties`

Common launch patterns ECC should handle:

- Vanilla, Paper, Purpur, Spigot: a primary server jar in the root folder
- Forge, NeoForge, Fabric: a loader-specific jar such as `forge*.jar`, `neoforge*.jar`, or `fabric-server*.jar`
- Custom Java launch: set `Executable` to `java` or `java.exe` and place the full server start command in `LaunchArgs`

Required game-side setup:

- Accept the Mojang EULA in `eula.txt`
- Enable RCON in `server.properties` if you want live player queries from ECC
- Set `rcon.port` and `rcon.password` to match the profile

If player queries fail:

- Confirm `enable-rcon=true` is set in `server.properties`
- Confirm the profile `RconPassword` matches the server password
- Check that the server is listening on the expected RCON port
...
- Usually auto-filled by ECC:
  ...
  - `StdinSaveCommand`
- You must verify manually:
  - `ConfigRoot` points to the real `%USERPROFILE%\\Zomboid\\Server`
  - `ServerLogRoot` points to the real `%USERPROFILE%\\Zomboid\\Logs`
  - `Executable` really matches your installed dedicated server start script
  - optional RCON fields are correct if you use them
- Recommended checks/toggles:
  - `EnableAutoRestart`: usually `On`
  - `DisableFileTail`: `Off`
  - `CaptureOutput`: usually `Off`
- Do not trust the profile yet if:
  - the live session log never appears
  - the Commands window spawner tools cannot load expected data
  - player detection never updates even though the server is running
...
- Usually auto-filled by ECC:
  ...
  - `ConfigRoots`
- You must verify manually:
  - `SatisfactoryApiToken` is present and valid if you use token auth
  - the API host/port match the actual server
  - `ConfigRoots` point at the folders you expect ECC to edit
  - your launch ports still match your server setup
- Recommended checks/toggles:
  - `EnableAutoRestart`: usually `On`
  - `DisableFileTail`: usually `Off`
  - `CaptureOutput`: only if you need extra console capture
- Do not trust the profile yet if:
  - `Test API` fails
  - save or shutdown calls fail from ECC
  - the API token was generated but never pasted into the profile
  - the dashboard never reflects live API state correctly
...
- Usually auto-filled by ECC:
  ...
  - `SaveMethod`
- You must verify manually:
  - world name, server name, password, public flag, and crossplay values in `LaunchArgs`
  - `ServerLogPath` points to the file you actually want ECC to read
  - optional `StdinPreferWindow` and `StdinWindowProcessName` values only if you really need them
- Recommended checks/toggles:
  - `EnableAutoRestart`: usually `On`
  - `DisableFileTail`: `Off` if the fixed log file is stable; `On` if you prefer activity-feed behavior
  - `CaptureOutput`: only if the fixed log file is not enough
- Do not trust the profile yet if:
  - the fixed log path never updates
  - the process starts but ECC cannot track it
  - you expect true player-aware automation, because Valheim still uses a more conservative setup in ECC
...
- ECC is built for one Windows host managing local server processes. It is not a multi-host orchestrator.
- Valheim still uses a more conservative player-aware setup in ECC than the other fully trusted game paths.
- Hytale currently does not have the same in-game announcement path as Minecraft, 7 Days to Die, Palworld, or Project Zomboid.
- Satisfactory and Valheim do not currently expose the same simple server-console-to-player broadcast flow used by the in-game announcement feature.
- `Full Restart` is intentionally disruptive: it stops running servers, closes ECC, and relaunches the full program.
